...
<body>
    ...
    <header>
        <h1>Écoles d’ingénieur</h1>
        <p>Retrouver les diplômes proposés par les écoles d'ingénieur</p>
    </header>
    ...
    $formation = $db->query('SELECT * FROM formation WHERE diplome = "Ingénieur" ORDER BY id');

    if(isset($_GET['q'])) {
        $q = htmlspecialchars($_GET{'q'});
        $formation = $db->query('SELECT * FROM formation  WHERE nom_complet LIKE "%'.$q.'%"  AND diplome = "Ingénieur" OR nom_etablissement LIKE "%'.$q.'%" AND diplome = "Ingénieur" OR initiale_alternance LIKE "%'.$q.'%" AND diplome = "Ingénieur" ORDER BY nom_complet ASC ' );
    }
        ?>
